...
class SemanticFieldEvolver:
    ...
    def _update_persistent_memory_adjoint(self, new_field: FSEField, decay_rate: float = 0.97):
            """
...
class ContinuousFieldSampler:
    ...
    def _build_sampler_parameters(self):
        """Initialize sampler parameters with mixed precision and CORRECTED final projection"""
        current_dtype = get_default_dtype()
        
        # --- CORRECTED: Ensure the final dimension is the vocab_size ---
        projection_dims = [
            self.channels,
            self.channels, # Keep dimensionality for a deeper projection
            self.channels // 2,
            self.vocab_size, # This is the critical change
        ]

        # Use float32 for the final layer's weights to prevent overflow, even in MP training
        final_layer_dtype = cp.float32
        ...
        for i in range(len(projection_dims) - 1):
            ...
            # Determine the correct dtype for the current layer
            layer_dtype = final_layer_dtype if out_dim == self.vocab_size else current_dtype

            W_data = self.backend.random.normal(0, init_scale, (in_dim, out_dim)).astype(layer_dtype)
            b_data = self.backend.zeros(out_dim, dtype=layer_dtype)

            self.parameters[f"projection_{i}"] = FSEField(W_data, FieldType.LINEAR, device=self.device, dtype=layer_dtype)
            self.parameters[f"bias_{i}"]       = FSEField(b_data, FieldType.LINEAR, device=self.device, dtype=layer_dtype)

        probe_init = self.backend.random.normal(0, 0.1, (self.channels, 1)).astype(current_dtype)
        self.coherence_probe = FSEField(probe_init, FieldType.LINEAR, device=self.device, dtype=current_dtype)
...
class FSULanguageModel:
    ...
    def backward(self, upstream_grads: Dict[str, FSEField], cache: Dict[str, Any]) -> Dict[str, Any]:
            """
            [LOGS CLEANED V2] Complete model backward pass using adjoint PDE solving.
            This version removes the high-frequency orchestration logs for a cleaner output.
            """
            # INFO log "Starting full adjoint orchestration" has been removed.
            
            all_param_grads = {}
            
            try:
                # 1. ADJOINT SAMPLER: Field-to-logits backward pass
                ...
                logger.debug("✅ ADJOINT Sampler backward complete")
                
                # 2. ADJOINT EVOLVER: Combine gradients and run multi-step backward pass
                initial_grad_for_evolver = grad_to_evolver
                
                if 'fsu_evolved_field' in upstream_grads:
                    # This logic correctly aligns gradients of different lengths.
                    coherence_grad = upstream_grads['fsu_evolved_field']
                    target_seq_len = initial_grad_for_evolver.shape[1]
                    
                    if coherence_grad.shape[1] > target_seq_len:
                        coherence_grad_data_truncated = coherence_grad.data[:, :target_seq_len, :]
                        coherence_grad_aligned = FSEField(
                            coherence_grad_data_truncated, coherence_grad.field_type,
                            device=coherence_grad.device, dtype=coherence_grad.dtype
                        )
                        initial_grad_for_evolver = initial_grad_for_evolver + coherence_grad_aligned
                    else:
                ...
                logger.debug("✅ ADJOINT Evolver backward complete")
                
                # 3. ADJOINT FRONTEND: Character-to-field backward pass
                ...
                logger.debug("✅ ADJOINT Frontend backward complete")
                
                # INFO log "Complete adjoint orchestration successful" has been removed.
    ...
    def _sample_from_probs(self, probabilities: ArrayLike) -> int:
        """Sample from probability distribution"""
        if self.device == "gpu": 
            return int(self.backend.random.choice(len(probabilities), p=probabilities))
        else: 
