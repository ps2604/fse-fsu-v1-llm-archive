...
class FSUDataProcessor:
    ...
    def _process_file_wrapper(self, args):
        parser_func, file_path, base_filename = args
        logger.info(f" spawning process for '{base_filename}'...")
        records = self._read_file_to_records(file_path)
        if not records: return []
        logger.info(f"  Processing {len(records)} records from '{base_filename}'...")
        start_time = time.time()
        processed_items = list(parser_func(records))
        processing_time = time.time() - start_time
        logger.info(f"✅ Finished '{base_filename}': Found {len(processed_items)} items in {processing_time:.2f}s.")
