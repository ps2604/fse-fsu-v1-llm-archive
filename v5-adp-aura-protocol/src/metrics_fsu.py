...
class FSUMetricsComputer:
    ...
    def _assess_logical_flow(self, text: str) -> float:
        ...
        try:
            ...
            return 0.5
# In FSUMetricsComputer class, add these new methods

    def compute_conversation_coherence(self, evolved_field: ArrayLike, target_chars_cpu: np.ndarray) -> Dict[str, float]:
        """Measures how well the model maintains context in conversations."""
        # This is a proxy that measures the cosine similarity between the two halves of the evolved field.
        try:
            seq_len = evolved_field.shape[1]
            first_half_mean = self.backend.mean(evolved_field[:, :seq_len//2, :], axis=1)
            second_half_mean = self.backend.mean(evolved_field[:, seq_len//2:, :], axis=1)
            
            first_half_norm = self.backend.linalg.norm(first_half_mean, axis=1, keepdims=True) + 1e-9
            second_half_norm = self.backend.linalg.norm(second_half_mean, axis=1, keepdims=True) + 1e-9
            
            similarity = self.backend.sum((first_half_mean / first_half_norm) * (second_half_mean / second_half_norm), axis=1)
            return {'fsu_conversation_coherence': float(self.backend.mean(similarity))}
        except Exception:
            return {'fsu_conversation_coherence': 0.0}
    ...
    def compute_reasoning_consistency(self, evolved_field: ArrayLike, target_chars_cpu: np.ndarray) -> Dict[str, float]:
        """
        [DEFINITIVE STABLE VERSION] Measures how consistently the model represents
        repeated concepts. This version adds normalization to prevent numerical
        instability and ensures the output is always in the [0, 1] range.
        """
        try:
            ...
            for i in range(target_chars_cpu.shape[0]):
                ...
                for concept in repeated_concepts:
                    ...
                    # --- THIS IS THE FIX ---
                    # Normalize the vectors before calculating variance to ensure stability.
                    # This prevents the numerical explosion that caused the negative value.
                    norms = self.backend.linalg.norm(field_vectors, axis=1, keepdims=True) + 1e-9
                    normalized_vectors = field_vectors / norms
                    # -----------------------
                    
                    mean_vector = self.backend.mean(normalized_vectors, axis=0)
                    # We want low variance, so we calculate similarity as (1 - variance)
                    variance = self.backend.mean(self.backend.sum((normalized_vectors - mean_vector)**2, axis=1))
                    
                    # Clamp the consistency score to be within the [0, 1] range.
                    consistency = max(0.0, 1.0 - float(variance))
                    total_consistency += consistency
                    num_concepts += 1
            
            return {'fsu_logical_consistency': float(total_consistency / (num_concepts + 1e-9))}
        except Exception:
