...
class FSULanguageModel:
    ...
    def forward(self, model_input: Union[ArrayLike, FSEField], training: bool = True) -> Tuple[Dict[str, FSEField], Dict[str, Any]]:
        """
        ✅ ADP-ADAPTED: Forward pass that now accepts either a character sequence OR
        a pre-compiled FSEField from the ADP data loader.
        """
        frontend_cache = {}
        
        # Check if the input is a character sequence (old path) or a pre-compiled field (new ADP path)
        if isinstance(model_input, FSEField):
            # ADP DATA PATH: Input is already a semantic field
            semantic_field = model_input
            logger.debug("ADP Training: Bypassing Lexical Frontend.")
        else:
            # ORIGINAL DATA PATH: Input is a character sequence, process through frontend
            character_sequence = model_input
        ...
        cache = {
            'input_sequence': model_input if not isinstance(model_input, FSEField) else None, 
            ...
            'config': self.config,
            'is_adp_input': isinstance(model_input, FSEField) # Flag for the backward pass
        }
    ...
    def backward(self, upstream_grads: Dict[str, FSEField], cache: Dict[str, Any]) -> Tuple[Dict[str, Any], FSEField]:
            ...
            try:
                ...
                # ADP-AWARE FIX:
                is_adp_input = cache.get('is_adp_input', False)

                if is_adp_input:
                    # ADP data bypassed frontend - no gradients needed
                    logger.debug("🔧 ADP input: skipping frontend gradient computation")
                    # Don't add 'lexical_frontend' to all_param_grads at all
                else:
                    # Character input - compute frontend gradients normally
