...
class FieldOperations:
    ...
    def _cupy_vectorized_im2col(input_padded, KH, KW, S_H, S_W, out_h, out_w):
        """CuPy-optimized vectorized im2col using advanced indexing - MEMORY SAFE"""
        ...
        try:
            ...
            logger.error(f"CuPy im2col failed: {e}")
    ...
    def _numpy_vectorized_im2col(input_padded, KH, KW, S_H, S_W, out_h, out_w):
        """NumPy-optimized vectorized im2col using stride_tricks - MEMORY SAFE"""
        ...
        try:
            ...
            logger.error(f"NumPy im2col failed: {e}")
            raise
    ...
    def _vectorized_col2im(grad_cols, input_padded_shape, cache):
        """Vectorized col2im operation for backward pass - MEMORY SAFE"""
        ...
        try:
            ...
            logger.error(f"Col2im operation failed: {e}")
...
class MorphSolver:
    ...
    def update_layer_weights(self, input_data: ArrayLike, target_indices: ArrayLike,
            ...
            try:
                ...
                theta = self.backend.linalg.solve(XtX, X_matrix.T @ Y_matrix)
                W_new = theta[:-1, :]
                b_new = theta[-1, :]
