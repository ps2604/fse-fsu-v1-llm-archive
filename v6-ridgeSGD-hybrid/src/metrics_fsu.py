...
from adjoint_core_optimized import FSEField, FieldType, get_default_dtype, ArrayLike
...
class FSUMetricsComputer:
    ...
    def compute_all_metrics(self, predictions: Dict[str, FSEField], targets: Dict[str, FSEField],
            ...
            """
            [DEFINITIVE FINAL VERSION] Computes the full suite of FSU language metrics,
            now including all field-based performance metrics for conversation, reasoning,
            and math, while preserving all original functionality.
            """
            metrics = {}
            try:
                target_chars_field = targets.get('fsu_character_targets')

                # === 1. Primary Language Modeling & Prediction Metrics ===
                if 'fsu_character_logits' in predictions and target_chars_field:
                    logits_field = predictions['fsu_character_logits']
                    
                    # Base LM metrics (Perplexity, Accuracy, etc.)
                    lm_metrics = self.compute_language_modeling_metrics(logits_field, target_chars_field)
                    metrics.update(lm_metrics)

                    # Mathematical Accuracy Metric (from logits)
                    math_metrics = self.compute_mathematical_accuracy(
                        logits_field.data,
                        target_chars_field.data
                    )
                    metrics.update(math_metrics)

                # === 2. Field Quality & Inferred Performance Metrics ===
                evolved_field_obj = predictions.get('fsu_evolved_field')
                if evolved_field_obj and target_chars_field:
                    evolved_field_data = evolved_field_obj.data
                    
                    # --- THIS IS THE FIX ---
                    # This correctly calls the semantic metrics function and renames the keys
                    # by removing the 'fsu_' prefix before adding the 'evolved_field_' prefix.
                    semantic_metrics = self.compute_semantic_field_metrics(evolved_field_obj, None)
                    field_specific_metrics = {f"evolved_field_{k.replace('fsu_', '')}": v for k, v in semantic_metrics.items()}
                    metrics.update(field_specific_metrics)
                    # -------------------------------------------------------------

                    # Get a CPU copy of the target text for analysis
                    target_cpu = target_chars_field.data.get() if hasattr(target_chars_field.data, 'get') else target_chars_field.data

                    # --- NEW: Conversation & Reasoning Performance Metrics ---
                    convo_metrics = self.compute_conversation_coherence(evolved_field_data, target_cpu)
                    metrics.update(convo_metrics)

                    reasoning_metrics = self.compute_reasoning_consistency(evolved_field_data, target_cpu)
                    metrics.update(reasoning_metrics)

                # === 3. Global & Other Metrics (Preserved) ===
                global_metrics = self.compute_global_fsu_metrics(predictions, targets)
                metrics.update(global_metrics)

                # This preserves your original logic for text-based metrics if they are ever used
                if generated_text and target_text:
                    text_based_metrics = self.compute_conversation_metrics(generated_text, target_text, conversation_context)
                    metrics.update(text_based_metrics)

                # === 4. Memory Optimization (Preserved) ===
    ...
    def compute_conversation_coherence(self, evolved_field: ArrayLike, target_chars_cpu: np.ndarray) -> Dict[str, float]:
        """
        [DEFINITIVE STABLE VERSION] Measures conversation coherence using tanh
        normalization to ensure the output is always in a meaningful [-1, 1] range.
        """
        try:
            seq_len = evolved_field.shape[1]
            if seq_len < 2: return {'fsu_conversation_coherence': 0.0}

            # --- THIS IS THE FIX ---
            # Apply tanh to the mean vectors to squash them into a stable [-1, 1] range
            # before calculating similarity. This prevents numerical explosion.
            first_half_mean = self.backend.tanh(self.backend.mean(evolved_field[:, :seq_len//2, :], axis=1))
            second_half_mean = self.backend.tanh(self.backend.mean(evolved_field[:, seq_len//2:, :], axis=1))
            # -----------------------

            # Normalize the vectors
            first_half_norm = self.backend.linalg.norm(first_half_mean, axis=1, keepdims=True) + 1e-9
            second_half_norm = self.backend.linalg.norm(second_half_mean, axis=1, keepdims=True) + 1e-9
            
            similarity = self.backend.sum((first_half_mean / first_half_norm) * (second_half_mean / second_half_norm), axis=1)
            
            # Ensure the final metric is in the [0, 1] range for interpretability
            final_coherence = (self.backend.mean(similarity) + 1.0) / 2.0

            return {'fsu_conversation_coherence': float(final_coherence)}
        except Exception:
            return {'fsu_conversation_coherence': 0.0}
...
# In file: metrics_fsu.py

    def compute_conversation_coherence(self, evolved_field: ArrayLike, target_chars_cpu: np.ndarray) -> Dict[str, float]:
        """
        [DEFINITIVE, FINAL VERSION] This metric now correctly measures the field's
        dynamic activity, aligning it with the new loss function. A value near 1.0
        indicates a healthy, dynamic field.
        """
        try:
            if evolved_field.shape[1] <= 1: return {'fsu_conversation_coherence': 0.0}

            # --- THIS IS THE FIX ---
            # Calculate the field's mean gradient magnitude, which we call "activity"
            field_diff = evolved_field[:, 1:, :] - evolved_field[:, :-1, :]
            grad_magnitude = self.backend.sqrt(self.backend.sum(field_diff**2, axis=-1))
            mean_activity = self.backend.mean(grad_magnitude)
            
            # The coherence score is now a measure of how close the activity is to an ideal target of 1.0
            # We use an exponential decay to map the distance to a [0, 1] score.
            coherence_score = self.backend.exp(-self.backend.abs(mean_activity - 1.0))
            # --- END FIX ---

            return {'fsu_conversation_coherence': float(coherence_score)}
        except Exception:
            return {'fsu_conversation_coherence': 0.0}



    def compute_reasoning_consistency(self, evolved_field: ArrayLike, target_chars_cpu: np.ndarray) -> Dict[str, float]:
        """
        [DEFINITIVE, CORRECTED VERSION] This metric now uses an exponential score
        based on the L2 distance, which is a sensitive and correct measure of
        how tightly clustered the representations for a concept are.
        """
        try:
            total_distance = 0.0
            num_concepts = 0
            for i in range(target_chars_cpu.shape[0]):
                # ... (concept finding logic is the same) ...
                text = bytes(c - 4 for c in target_chars_cpu[i] if c > 3).decode('utf-8', 'ignore')
                ...
                for concept in repeated_concepts:
                    indices = [m.start() for m in re.finditer(r'\b' + re.escape(concept) + r'\b', text.lower())]
                    safe_indices = [idx for idx in indices if idx < evolved_field.shape[1]]
                    if len(safe_indices) < 2: continue
                    
                    field_vectors = evolved_field[i, safe_indices, :]
                    
                    # --- THIS IS THE FIX ---
                    # Calculate the average L2 distance from the mean, same as the loss function.
                    mean_vector = self.backend.mean(field_vectors, axis=0, keepdims=True)
                    distance = self.backend.mean(self.backend.sqrt(self.backend.sum((field_vectors - mean_vector)**2, axis=1)))
                    total_distance += distance
                    # --- END FIX ---
                    num_concepts += 1
            
            if num_concepts == 0:
                return {'fsu_logical_consistency': 0.0}
            
            # Convert the average distance to a score from 0 to 1
            # A lower distance means higher consistency/score.
            avg_distance = total_distance / num_concepts
            consistency_score = self.backend.exp(-avg_distance)

            return {'fsu_logical_consistency': float(consistency_score)}
        except Exception:
            return {'fsu_logical_consistency': 0.0}

    def compute_mathematical_accuracy(self, logits: ArrayLike, targets: ArrayLike) -> Dict[str, float]:
            """
            [DEFINITIVE FINAL VERSION V2] Calculates accuracy on the final number in
            math problems using robust text decoding and regex. This version fixes
            a critical dtype error in the target labels.
            """
            try:
                predictions = self.backend.argmax(logits, axis=-1)

                # Convert raw target FSEField data to a CPU numpy array for processing
                target_cpu = targets.get() if hasattr(targets, 'get') else targets

                # The target tokens must be integers for bytearray decoding.
                target_cpu_int = target_cpu.astype(np.int32)

                mask = np.zeros_like(target_cpu_int, dtype=np.float32)

                # Loop through each item in the batch to build the mask
                for i in range(target_cpu_int.shape[0]):
                    try:
                        # This line will now work correctly with integer tokens
                        text_bytes = bytearray(c - 4 for c in target_cpu_int[i] if c >= 4)
                        text = text_bytes.decode('utf-8', 'ignore')
                    except Exception:
                        continue # Skip samples that fail to decode

                    numbers = [m for m in re.finditer(r'\b\d+\.?\d*\b', text)]
                    if numbers:
                        last_num_match = numbers[-1]
                        start_byte, end_byte = last_num_match.span()
                        if end_byte <= mask.shape[1]:
                            mask[i, start_byte:end_byte] = 1.0

                # The rest of the calculation proceeds as before
                mask_gpu = self.backend.asarray(mask)
                correct = (predictions == self.backend.asarray(target_cpu_int)).astype(self.backend.float32)
                
                masked_correct = correct * mask_gpu
                total_masked_tokens = self.backend.sum(mask_gpu)

                if total_masked_tokens > 0:
                    accuracy = self.backend.sum(masked_correct) / total_masked_tokens
                else:
                    accuracy = 0.0
                
                return {'fsu_mathematical_accuracy': float(accuracy)}
            except Exception:
                return {'fsu_mathematical_accuracy': 0.0}
