...
class FSEAdjointSolvers:
    ...
    def solve_forward_pde(self, 
        ...
        """
        This is the clean, production version of the solver, with the proven
        internal tanh clamp for step-by-step stability.
        """
        ...
        for step in range(num_steps):
            # 1. Compute the field derivative
            field_derivative = self._compute_linguistic_field_operator(
                current_field, parameters, field_type, step * dt
            )
            
            # 2. Integrate with a simple Euler step
            current_field = self._euler_integration_step(current_field, field_derivative, dt)

            # 3. Stabilize the field after each integration step using a tanh clamp.
            current_field.data = self.backend.tanh(current_field.data)
            
            trajectory.append(current_field)

        evolution_cache['num_steps'] = num_steps
